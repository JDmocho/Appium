...
class AccountLogin(unittest.TestCase):
    ...
    CUR_DIR = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(CUR_DIR, '..', 'data/correct_data_account.csv')

    @data(*GetData.get_data(file_path))
    @unpack

    def test_correct_data(self, email, password):
        ...
        testerka = self.driver.find_elements_by_android_uiautomator('new UiSelector().textContains("Testerka")')
